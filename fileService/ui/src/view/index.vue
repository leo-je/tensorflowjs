...
export default defineComponent({
    ...
    methods: {
        ...
        packageUatApp() {
            ...
            http('post', '/api/packageUatApp', { branch: _this.branch }).then(function (data) {
                console.log(data)
                ElMessage(data.msg)
            })
        },
        packageDebugApp() {
            let _this = this;
            console.log(_this.branch);
            http('post', '/api/packageDebugApp', { branch: _this.branch }).then(function (data) {
                console.log(data)
                ElMessage(data.msg)
            })
        },
        packageProdApp() {
            let _this = this;
            http('post', '/api/packageProdApp', { branch: _this.branch }).then(function (data) {
                console.log(data)
                ElMessage(data.msg)
            })
        },
        ...
        deleteRow(index: any, rows: any) {
            let _this = this;
            http('post', '/api/delete/' + rows[index].fileName)
                .then(function (data) {
                    _this.getFileList()
                })
        },
        ...
        keysort(key: string, sortType: boolean) {
            let fn = function (a: any, b: any) {
                ...
                return sortType ? ~~(new Date(Date.parse(a[key])) < new Date(Date.parse(b[key]))) : ~~(new Date(Date.parse(a[key])) > new Date(Date.parse(b[key])));
            }
            return fn;
        }
    },
